...
function sendMessage() {
  try {
    ...
    console.log('[NAVIgit] Sending question to AI:', text);
    
    fetchAIReply(text)
      .then(result => {
        console.log('[NAVIgit] AI reply received:', result);
        removeTypingIndicator(typingEl);
        appendMessage('ai', result.answer, result.sources);
      })
      .catch(err => {
        console.error('[NAVIgit] AI Chat Error:', err);
        removeTypingIndicator(typingEl);
        appendMessage('ai', 'Error: ' + (err.message || 'Could not connect to AI. Please try again.'), []);
      });
  } catch (err) {
    console.error('[NAVIgit] Critical Chat logic crash:', err);
  }
}
...
function appendMessage(role, text, sources = []) {
  ...
  // Safe simple "markdown" formatting (bold and newlines)
  if (role === 'ai') {
    let formatted = text
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
...
async function startStatusPolling() {
  ...
  console.log('[NAVIgit] Starting RAG status polling...');
  resetRagState();

  pollingInterval = setInterval(async () => {
    try {
      const res = await fetch(RAG_API_BASE + '/rag/status');
      if (!res.ok) return;
      
      const data = await res.json();
      if (data.indexed) {
        console.log('[NAVIgit] RAG Index is ready!');
        onRagIndexReady();
        clearInterval(pollingInterval);
        pollingInterval = null;
      }
    } catch (err) {
      console.warn('[NAVIgit] Status polling failed:', err);
    }
  }, 3000); // Poll every 3 seconds
}
window.startStatusPolling = startStatusPolling;

async function indexRepo(githubUrl) {
  // Deprecated: No longer called from main.js as backend handles indexing in background.
  // But kept here for reference or manual triggers if needed.
  if (!githubUrl) return;
  startStatusPolling();
}
