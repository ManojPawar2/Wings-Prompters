...
def architectural_scorer(path: str) -> int:
    ...
    # +3 if inside key directories (check all parts for monorepo support)
    if any(p in {"src", "server", "backend", "app", "main", "core"} for p in parts):
        score += 3
        
    # +2 if relative shallow depth
    if len(parts) <= 4:
        score += 2
...
def noise_filter(path: str) -> bool:
    ...
    # Ignore storybook/style only
    if "storybook" in lower or "stories." in lower or "style" in lower or "theme" in lower:
        return False
...
def _resolve_java_import(raw: str, all_paths_set: set[str]) -> str | None:
    """
    Resolves a Java import (e.g., 'org.springframework.boot.SpringApplication')
    by looking for a matching .java file in the entire repository tree.
    """
    as_path = raw.replace(".", "/") + ".java"
    # 1. Try exact match
    if as_path in all_paths_set:
        return as_path
    # 2. Try common prefix
    candidate = "src/main/java/" + as_path
    if candidate in all_paths_set:
        return candidate
    # 3. Search for any path that ends with this class path
    for p in all_paths_set:
        if p.endswith("/" + as_path):
            return p
    return None
...
def graph_pruner(adjacency: dict[str, list[str]]) -> tuple[dict[str, GraphNode], list[GraphEdge]]:
    ...
    # Keep top 200
    top_paths = {p for p, _ in scored[:200]}
