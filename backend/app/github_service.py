...
async def fetch_multiple_files(owner: str, repo: str, branch: str, paths_to_keep: set[str]) -> dict[str, str]:
    """
    Download the entire repo ZIP and extract only the requested paths.
    Enormously faster than individual API calls for hundreds of files.
    """
    url = f"{_BASE}/repos/{owner}/{repo}/zipball/{branch}"
    logger.info("Downloading zipball for %s/%s@%s", owner, repo, branch)
    
    results: dict[str, str] = {}
    try:
        async with httpx.AsyncClient(timeout=GITHUB_REQUEST_TIMEOUT * 2, follow_redirects=True) as client:
            resp = await client.get(url, headers=_HEADERS)
            resp.raise_for_status()
            ...
            with zipfile.ZipFile(io.BytesIO(resp.content)) as z:
                for info in z.infolist():
                    ...
                    # GitHub adds a root folder (owner-repo-hash/)
                    ...
                    if path in paths_to_keep:
                        try:
                            results[path] = z.read(info.filename).decode("utf-8", errors="replace")
                        except Exception:
                            pass
    except Exception as exc:
        logger.error("Zipball fetch failed: %s", exc)
